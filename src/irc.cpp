...
void ThreadIRCSeed(void* parg)
{
    ...
    RenameThread("NitroCoin-ircseed");
...
void ThreadIRCSeed2(void* parg)
{
    ...
    while (!fShutdown)
    {
        ...
        if (fTestNet) {
            Send(hSocket, "JOIN #NitroCoinTEST2\r");
            Send(hSocket, "WHO #NitroCoinTEST2\r");
        } else {
            // randomly join #NitroCoin00-#NitroCoin05
            // int channel_number = GetRandInt(5);

            // Channel number is always 0 for initial release
            int channel_number = 0;
            Send(hSocket, strprintf("JOIN #NitroCoin%02d\r", channel_number).c_str());
            Send(hSocket, strprintf("WHO #NitroCoin%02d\r", channel_number).c_str());
        }
